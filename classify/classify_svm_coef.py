...
def dostuff(k_val):
    ...
    '''
    selectFeats = getBestFeatsFromSvm(k_val)
    print selectFeats

    for label in range(3):
        print "****FOR CLASS %d" % (label)
        feats2 = []
        selectFeats1 = selectFeats[label]
        for feat in feats:
            feat1 = [float(feat[selectFeat]) for selectFeat in selectFeats1]
            feats2.append(feat1)
        printFreq(feats2, selectFeats1)
...
def printFreq(feats2, selectFeats1):
    computeFreq(feats2)
    prodMap = getProdMap(sys.argv[2])
    for i in range(len(selectFeats1)):
        prod = prodMap[selectFeats1[i]]
